# 💾 Автоматичні резервні копії (Backup) бази даних

## 📋 Опис

Після кожного коміту автоматично створюється резервна копія БД (`bot_data.db`) на
...
### Видалення старих резервних копій вручну
```powershell
# PowerShell
...
### 3. Вибери відповідний backup (зазвичай найновіший або останній перед помилкою)
```bash
python backup_db.py restore bot_data_704f186_20260404_200000.db
```
...
**Що відбувається:**
- ✅ Кожний комміт автоматично створює снімок БД
- ✅ Останні 15 файлів зберігаються в `backups/`
- ✅ Старші файли автоматично видаляються
- ✅ Ім'я файлу містить commit hash для ідентифікації

**Рекомендації:**
- 📦 Час від часу завантажуй резервні копії на зовнішній диск
...
| `backup_db.py` | Основний скрипт | Резервування БД |
...
# 1. Переконайся, що Python встановлено
python --version
